...
import type { RegisterExitPort } from '../../../ports/in/parking/register-exit.port';
...
@Injectable()
export class RegisterExitUseCase implements RegisterExitPort {
  constructor(
  ...
  async execute(input: RegisterExitInput): Promise<ParkingSession> {
    ...
    const result = await this.parkingPolicy.registerExit({
